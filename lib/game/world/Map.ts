...
export class Map {
  ...
  constructor(name: string, mapData?: any) {
    this.name = name;
  ...
  private createEnvironmentObjects() {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      tree.x = Math.random() * this.width;
      tree.y = Math.random() * this.height;
    ...
    for (let i = 0; i < 5; i++) {
      ...
      rock.x = Math.random() * this.width;
      rock.y = Math.random() * this.height;
