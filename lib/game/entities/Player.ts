...
export class Player {
  ...
  private get app() {
    return PIXI.Application.shared;
  }
  ...
  public update(deltaTime: number, inputHandler: any) {
    ...
    if (inputHandler.isPressed('KeyW') || inputHandler.isPressed('ArrowUp')) {
      dy = -this.speed;
      newDirection = 'up';
    } else if (inputHandler.isPressed('KeyS') || inputHandler.isPressed('ArrowDown')) {
      dy = this.speed;
      newDirection = 'down';
    } else if (inputHandler.isPressed('KeyA') || inputHandler.isPressed('ArrowLeft')) {
      dx = -this.speed;
      newDirection = 'left';
    } else if (inputHandler.isPressed('KeyD') || inputHandler.isPressed('ArrowRight')) {
      dx = this.speed;
      newDirection = 'right';
    }
    ...
    if (dx !== 0 || dy !== 0) {
      ...
      if (this.sprite.x !== previousX || this.sprite.y !== previousY) {
        this.hasMoved = true;
        this.gameStore.getState().updatePlayerPosition(
          this.sprite.x, 
          this.sprite.y, 
          this.direction
        );
